...
BOUNDS = [
    ...
    (18, 34),       # peak_age -- TEST: widened from (20,30) after Tommy flagged
                    # that REB/BLK/TOV all landed within 0.5yr of the old 20.0
                    # floor, the exact same boundary-pinning bug class already
                    # found and fixed once before in v1's blended curve.
    (-0.08, 0.08),  # slope_up (per year) -- TEST: un-floored at 0 too, so a
                    # real negative pre-peak trend (if any) isn't structurally
...
if __name__ == "__main__":
    ...
    with open(ROOT / "data" / f"kalman_fit_{stat_col}_widebounds_test.json", "w") as f:
