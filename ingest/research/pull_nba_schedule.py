...
{"generated":..., "tv": {date: {"AWAY@HOME": "NBC"}} (national broadcasts only), "games": {"2026-10-20": [["BOS","DET","19:00Z"], ...]}}  (away, home, tip-off UTC).  Used for schedule-aware roster planning
...
while d <= END:
    ...
    for ev in j.get("events", []):
        ...
        if h and a and h["team"].get("abbreviation") and a["team"].get("abbreviation"):
            names = [n for b in ev["competitions"][0].get("broadcasts", []) if b.get("market") == "national" for n in b.get("names", [])]
            if names:
                tvd[FIX.get(a["team"]["abbreviation"], a["team"]["abbreviation"]) + "@" + FIX.get(h["team"]["abbreviation"], h["team"]["abbreviation"])] = "/".join(names)
    ...
    d += timedelta(days=1)
    time.sleep(0.15)
out = {"generated": datetime.now(timezone.utc).isoformat(), "games": games, "tv": tv}
