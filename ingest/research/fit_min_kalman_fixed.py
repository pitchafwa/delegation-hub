...
BOUNDS = [
    ...
    (18, 34),       # peak_age -- widened, same fix as the other stats' retest
    (0.0, 4.0),     # slope_up (per year) -- FIX: MIN is raw minutes (0-40 scale),
                    # not a 0-1ish per-minute rate. The old (0.0,0.08) bound was
                    # copy-pasted from the rate-stat scale and silently capped
                    # MIN's real growth at ~40x too small (real within-player
                    # data: +1.5 to +2.4 min/game per year for developing
                    # players). Widened to a scale that can actually represent it.
    (-4.0, 0.0),    # slope_down (per year) -- same rescale for the decline side
]
...
if __name__ == "__main__":
    ...
    with open(ROOT / "data" / f"kalman_fit_{stat_col}_rescaled_test.json", "w") as f:
